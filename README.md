...
- 🎯 Entry points to execute your program using `python -m <smart_life_organizer>` or `$ smart_life_organizer` with basic CLI argument parsing.
...
from source
```bash
git clone https://github.com/author_name/project_urlname smart_life_organizer
cd smart_life_organizer
make install
```
...
```bash
pip install smart_life_organizer
```

## Executing

```bash
$ smart_life_organizer run --port 8080
```

or

```bash
python -m smart_life_organizer run --port 8080
```

or

```bash
$ uvicorn smart_life_organizer:app
```

## CLI

```bash
❯ smart_life_organizer --help
Usage: smart_life_organizer [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ smart_life_organizer create-user --help
Usage: smart_life_organizer create-user [OPTIONS] USERNAME PASSWORD
...
```bash
smart_life_organizer create-user admin admin --superuser
```
...
Run with `smart_life_organizer run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `smart_life_organizer create-user`.
...
-----------------------------------------------------
smart_life_organizer/__init__.py              4      0   100%
smart_life_organizer/app.py                  16      1    94%
smart_life_organizer/cli.py                  21      0   100%
smart_life_organizer/config.py                5      0   100%
smart_life_organizer/db.py                   10      0   100%
smart_life_organizer/models/__init__.py       0      0   100%
smart_life_organizer/models/content.py       47      1    98%
smart_life_organizer/routes/__init__.py      11      0   100%
smart_life_organizer/routes/content.py       52     25    52%
smart_life_organizer/routes/security.py      15      1    93%
smart_life_organizer/routes/user.py          52     26    50%
smart_life_organizer/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in smart_life_organizer/default.toml.
...
```bash
PROJECT_NAME_ENV=production smart_life_organizer run
```
